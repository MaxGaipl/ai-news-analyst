...
def demonstrate_api_responses():
    ...
    for name, bias, cred, status in sources:
        source = NewsSourceDefault(
            name=name, bias=bias, credibility=cred, status=status
        )
...
def benchmark_serialization():
    ...
    print(
        f"   Improvement:           {((default_time - optimized_time) / default_time * 100):.1f}% faster"
    )
...
def demonstrate_analytics_impact():
    ...
    sources = [
        ...
        NewsSourceOptimized(
            name="CNN",
            bias=BiasRating.LEFT,
            credibility=CredibilityRating.HIGH,
            status=VerificationStatus.VERIFIED,
        ),
        NewsSourceOptimized(
            name="Fox",
            bias=BiasRating.RIGHT,
            credibility=CredibilityRating.MIXED,
